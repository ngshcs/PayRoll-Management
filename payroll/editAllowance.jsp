	...
	else if("admin".equals(type))
	{
		...
		if( id!=null && name!=null && desc!=null )
		{
			try
			{
				...
				String query = "select name from allowance where id=" + id;
				ResultSet rs = stmt.executeQuery(query);
				rs.next();
				String old = rs.getString(1);

				query = "update allowance set name='" + name + "',description='" + desc+ "' where id=" + id;
				...
				while(rs.next())
				{
					...
					for(int i=0; i<c; i++)
					{
						...
						allowances = employee.getElementsByTagName("allowances");
						allowance= allowances.item(0);
